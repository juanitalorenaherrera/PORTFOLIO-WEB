# 👩‍💻 Juanita Lorena Herrera Herrera - Portafolio

¡Hola! Soy **Juanita Herrera**, apasionada del desarrollo web y la tecnología.  
Me especializo en **HTML, CSS, JavaScript y React**, con experiencia en proyectos innovadores. 🚀

---

## 🌟 Sobre Mí
Después de dedicar dos años al voluntariado, decidí sumergirme en el mundo de la programación.  
Actualmente, he trabajado en proyectos como:

### 🛡️ CryptoGuard
Aplicación de cifrado de texto desarrollada durante el programa **ONE de Oracle y Alura**.  
🔗 [Ver código en GitHub](https://github.com/juanitalorenaherrera/CryptoGuard)

### 🎲 Juego Secreto
Juego interactivo para adivinar un número secreto.  
🔗 [Ver código en GitHub](https://github.com/juanitalorenaherrera/JuegoSecreto)

### 🧠 Juego de Memoria
Juego de cartas interactivo para entrenar la memoria usando HTML, CSS y JavaScript.  
🔗 [Ver código en GitHub](https://github.com/juanitalorenaherrera/JuegoMemoria)

### 🚑 AmbuAlert
Plataforma web de emergencias con frontend y backend integrados, diseñada para ofrecer respuestas rápidas y organizadas.  
🔗 [Ver código en GitHub](https://github.com/juanitalorenaherrera/AmbuAlert)

---

## 🛠️ Habilidades

**Lenguajes y Tecnologías:**  

<p align="center">
  <img src="https://skillicons.dev/icons?i=html,css,js,react,nodejs,cs,dotnet,git,github,mongodb,vscode" />
</p>

**Diseño UX/UI:** Creación de interfaces intuitivas y funcionales  
**Seguridad Web:** Aplicación de prácticas básicas de protección  
**Soft Skills:** Trabajo en equipo, creatividad, adaptación al cambio

---

## 📜 Certificaciones

🎓 **Oracle Cloud Infrastructure Foundations Associate**  
🎓 **Oracle Cloud Infrastructure – Fundamentos**  
🎓 **OCI – Seguridad y Gobernanza**  
🎓 **Spring Boot 3 – API REST**  
🎓 **Spring Boot – Seguridad en API**  
🎓 **Java – Persistencia con JPA**  
🎓 **Java – API conectada al Frontend**  
🎓 **Java – Orientación a Objetos**

> Todos los certificados se muestran en la sección correspondiente de mi portafolio web y están disponibles como imágenes y PDFs descargables.

---

## 📫 Contacto

📩 **Email:** juanitaherrera846@gmail.com  
💼 **LinkedIn:** [juanita-herrera](https://www.linkedin.com/in/juanita-herrera-329064277/)  
🐙 **GitHub:** [juanitalorenaherrera](https://github.com/juanitalorenaherrera)

---

> Este README se actualizará a medida que agregue nuevos proyectos, certificaciones y mejoras a mi portafolio web.
